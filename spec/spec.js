...
describe('/api', () => {
	...
	describe('/topics', () => {
		it('get request to topics return 200 and arr of length of two', () => {
			return request.get('/api/topics').expect(200).then((res) => {
	...
	describe('/topics/:topic_slug/articles', () => {
		it('get request to articles return status 200 and arr of length of two and correct keys', () => {
			return request.get('/api/topics/mitch/articles').expect(200).then((res) => {
		...
		it('get request with a param that does not exist returns status 404 & correct msg', () => {
			return request.get('/api/topics/turtles/articles').expect(404).then((res) => {
	...
	describe('/articles/:article_id', () => {
		...
		it('get request with a valid param that does not exist returns status 404 & correct msg', () => {
			return request.get(`/api/articles/${userDocs._id}`).expect(404).then((res) => {
				expect(res.body.msg).to.equal('Invalid Param');
			});
		});
		it('patch returns 200 and updated vote count', () => {
			return request.patch(`/api/articles/${artDocs._id}?vote=up`).expect(200).then((res) => {
	...
	describe('/users/:username', () => {
		it('return single user info when passed an valid username & status 200', () => {
			return request.get(`/api/users/${userDocs.username}`).expect(200).then((res) => {
	...
	describe('articles/:article_id/comments', () => {
		it('return all comments for article matching id when given a valid id & status 200', () => {
			return request.get(`/api/articles/${artDocs._id}/comments`).expect(200).then((res) => {
	...
	describe('/comments/:comment_id', () => {
		it('DELETE returns a status 200 and removed the house', () => {
			return request.delete(`/api/comments/${commentDocs._id}`).expect(200).then((res) => {
